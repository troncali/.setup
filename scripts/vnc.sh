...
ORIG_PATH="$PATH"
X11_LOC="opt/X11"
BREWED_LOC="/usr/local/opt" # Assumes dependencies are installed by homebrew.
...
## TODO: Server build fails; configuration needs help; see also https://www.xquartz.org/Developer-Info.html

#mkdir unix
#cp -R source/unix/xserver unix/
#git clone git://github.com/XQuartz/xorg-server.git xorg-server # Change to download tarball
#cd xorg-server && git checkout server-1.16-apple
#cd ../ && cp -R xorg-server/* unix/xserver/
#cp -R /opt/X11/include/xorg/* unix/xserver/
#cd unix/xserver/
...
#
#make TIGERVNC_SRCDIR="$ASSETS"/TigerVNC/source
